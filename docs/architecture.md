...
2. LangGraph `orchestrator` runs context/intent nodes, then ReAct: `agent_react_step` → `pa_decision_step` (Pydantic AI, `output_type=Plan`, spreadsheet tools via `pa_tools`).
   - `context_analyzer` computes `DataContext` (per-table column stats, shared discriminators) from the submitted tables.
   - `intent_analyzer` is an async batch LLM classification step, not a pass-through: one call covers every table in `DataContext`, backfilling `topic`/`description`/`granularity` on each `TableProfile`. Results are cached in-process by `table_id` + a structural signature (column names/types/row count), so unchanged tables skip re-classification on later turns. It fails open — on error/timeout the three fields stay `None` and the graph proceeds — and refreshes the already-injected Data profile transcript message so `llm_decide` sees the backfilled fields in the same turn (`app/agent/sub_agents/intent_analyzer.py`, `app/agent/context_assembler.py::refresh_data_profile_message`).
...
5. Sync `/api/agent` uses the same graph; `/api/agent-stream` mirrors steps as SSE ([agent-stream-sse.md](./agent-stream-sse.md)). Preview confirm/abort/revise stay on sync `/api/agent` only.
