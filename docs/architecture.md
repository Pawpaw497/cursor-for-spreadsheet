...
| Agent (SSE) | `POST /api/agent-stream` | Same orchestrator as sync; SSE tool/plan/clarification events — see [agent-stream-sse.md](./agent-stream-sse.md) |
...
| `agentProjectPlan.ts` | Stream → `AgentProjectPlanResult` mapping (sync parity) |
...
5. Sync `/api/agent` and `/api/agent-stream` share the same graph and terminal payloads; the stream emits `tool_call` / `tool_result` steps plus one terminal event (see [agent-stream-sse.md](./agent-stream-sse.md)).
