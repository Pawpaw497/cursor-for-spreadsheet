...
> **状态（2026-06）**：下文大量「目标」已在主分支落地。生产路径请优先查阅 [`docs/architecture.md`](architecture.md)、[`docs/agent-memory.md`](agent-memory.md)、[`docs/agent-preview-lifecycle.md`](agent-preview-lifecycle.md) 与根目录 `README.md`。本文档保留为历史路线图与尚未完成项的索引；第三节起部分「现状」段落可能落后于代码。

**已落地摘要**

| 能力 | 状态 | 参考 |
|------|------|------|
| Agent 骨架（State / actions / PA decision / LangGraph） | ✅ | `server/app/agent/` |
| `/api/agent` + `/api/agent-stream` + 工具调用 | ✅ | `api/routes/agent.py` |
| 多轮 `history` + `appliedPlansSummary` + workspace memory | ✅ | [`agent-memory.md`](agent-memory.md) |
...
- **已修复**：add_column 的 expression 若为 LLM 返回的完整箭头（`row => body`），前后端会先 strip 为 body 再执行，避免 Apply 后新列为空；服务端用 SimpleNamespace 包装 row 以支持 `row.列名` 形式。
...
- **示例加载退避策略优化**：前端在调用 `/api/load-sample` 时改为较短的单次超时与有限重试，避免在后端未启动时卡在「正在加载示例…」长达数十秒；同时保留少量重试以覆盖「后端刚启动中」的场景。
- **导入大文件的用户反馈**：上传 Excel/CSV 时，为 `/api/import-file` 使用更长的超时窗口，并在状态栏明确提示「超过约 20 秒可检查文件大小或后端日志」，减少用户对前端卡死的误判。
- **解析层可观测性增强**：在 `load.py` 中为 Excel/CSV 解析增加耗时与行列统计日志，并在解析函数中引入可选的最大行数参数，为后续实现「仅预览前 N 行」的大文件策略打下基础。
