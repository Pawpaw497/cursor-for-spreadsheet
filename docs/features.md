...
- SSE：`tool_call` / `tool_result` 步骤事件 + 终端 `plan_done`、`preview_ready`、`clarification`、`finish`；可选 `VITE_AGENT_USE_STREAM=true` 走 `/api/agent-stream`（与同步 `/api/agent` 结果对齐）。详见 [`agent-streaming.md`](agent-streaming.md)。
